    ...
    for ii in range(len(json_file_list)):
        ...
        for single_img_data in tqdm(data):
            try:
                ...
                os.symlink(os.path.join('{}'.format(image_root_list[ii]), '{}'.format(single_img_data['filename']) ), 
                           os.path.join('{}'.format(ln_image_dir), '{}'.format('%04d'%(ii)+'_'+'%06d'%(single_img_data_count)+single_img_data['filename'][-4:])) )
        ...
        for vd in new_data[int(len(data)*train_val_split):]:
            ...
            os.symlink( os.path.join('{}'.format(ln_image_dir), '{}'.format(vd['filename']) ), 
                        os.path.join( '{}'.format(valid_image_root), '{}'.format(vd['filename']) ) )
