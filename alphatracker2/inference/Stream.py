...
def real_time_tracking(detector, estimator, nClasses, max_pid_id_setting, save=False, savepath='', fps=15, size=(640,480)):
    ...
    inputResH = int(320/1) # 320
    inputResW = int(256/1) # 256
    outputResH = int(80/1) # 80
    outputResW = int(64/1) # 64
    results = []
    fps_per_frame = []
    tracked_ = []
    nd = []
        ...
        while 1:
            ...
            if boxes.shape[0] > 0:
                ...
                if torch.cuda.is_available():
                    inps = inps.cuda()
                with torch.no_grad():
                    hm = estimator(inps); #print(hm.shape)
            ...
            if counter_ > 0:
                cols = {0: (255, 0, 0), 1: (0, 255, 0), 2: (0,0,255), 3: (255,255,255)}
                tracked1, not_detected1 = Infer.track(results, max_pid_id_setting)
                #print(tracked1)
                #if counter_ >= 1:
                    #print(type(result), result)
                #    if len(result['result']) != 0:
                #        rr = result['result'][0]['keypoints'].numpy()
                        
                #        for cls in range(nClasses):
                #            print(rr[cls][0])
                #            cv2.circle(to_plot, ( int(rr[cls][0]), int(rr[cls][1]) ), 15, cols[cls], -1)
                ...
                #            for cc, bp in enumerate(obj[keys_in_2]['box_pose_pos']):
                                #twotwo = cols[pid_for_this_one]
                                #twotwo = (twotwo[0], twotwo[1], cc*50)
                #                cv2.circle(to_plot, (int(bp[0]), int(bp[1])), 15, cols[cc], -1)
...
#onnx_model = onnx.load(r"C:\Users\AneeshBal\Desktop\super_resolution.onnx")
...
def real_time_tracking_onnx(detector, estimator, nClasses, max_pid_id_setting, save=False, savepath='', fps=15, size=(640,480)):
    ...
    width = 480.0#vid.get(cv2.CV_CAP_PROP_FRAME_WIDTH)   # float
    height = 320.0 #vid.get(cv2.CV_CAP_PROP_FRAME_HEIGHT) # float
    ...
    try:
        ...
        while 1:
            ...
            if boxes.shape[0] > 0:
                ...
                ort_inputs = {ort_session.get_inputs()[0].name: to_numpy(inps)}
                hm = ort_session.run(None, ort_inputs)
                hm = hm[0]
